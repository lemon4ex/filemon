...
// https://opensource.apple.com/source/xnu/xnu-7195.81.3/bsd/sys/fsevents.h
#include "fsevents.h"
//#include "colors.h"
...
#define COLOR_OP COLOR_YELLOW
#define COLOR_PROC COLOR_BLUE
#define COLOR_PATH COLOR_CYAN
	...
	//		if (offInBuf) { printf ("Next event: %d\n", offInBuf);};
                    ...
                    && interesting_file (fse_arg->data, fileFilters, numFileFilters))
		   {
		   	printf ("%5d %s%s%s\t%s%s%s ", fse->pid, 
				color ? COLOR_PROC: "" , procName,  color ? COLOR_NORMAL : "",
				color ? COLOR_OP : "", typeToString(fse->type), color ? COLOR_NORMAL : "" );

		        // The path name is null terminated, so that's cool
		        printf ("%s%s%s\t", 
				color ? COLOR_PATH : "" , fse_arg->data, color ? COLOR_NORMAL :"");
